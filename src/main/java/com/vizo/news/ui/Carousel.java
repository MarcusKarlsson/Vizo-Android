...
public class Carousel extends ViewGroup {
    ...
    private void reset() {
        ...
        if (mSelection > mAdapter.getCount() - 1)
            mSelection = mAdapter.getCount() - 1;
...
//    }
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mTouchState == TOUCH_STATE_SCROLLING) {
                    ...
                    if (xMoved && xDiff > yDiff) {
                        ...
                        enableChildrenCache();
                        cancelLongPress();
                    } else if (yDiff > xDiff && isDragging == false) {
                        isDragging = true;
                        if (mDragListener != null) {
                            mDragListener.onStartDrag(getSelection());
                        }
                    }
                }
                break;
    ...
    public static interface CarouselDragListener {
        public void onStartDrag(int position);
    }

    private CarouselDragListener mDragListener;

    public void setDragListener(CarouselDragListener dragListener) {
