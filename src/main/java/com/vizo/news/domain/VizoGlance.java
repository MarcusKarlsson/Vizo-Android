...
public class VizoGlance implements Parcelable {
     ...
     */
    public static final Parcelable.Creator<VizoGlance> CREATOR = new Parcelable.Creator<VizoGlance>() {
        public VizoGlance createFromParcel(Parcel in) {
            return new VizoGlance(in);
        }

        public VizoGlance[] newArray(int size) {
            return new VizoGlance[size];
        }
    };
    ...
    /**
     * Get brief description for facebook sharing
     *
     * @return Brief description to be posted to Facebook
     */
    public String getFacebookDescription() {
        String shortDescription = description;
        if (description != null && description.length() > 100) {
            shortDescription = String.format("%s...", description.substring(0, 100));
        }
        return shortDescription;
    }

    public String getFacebookFullDescription() {
        String shortDescription = description;
        return shortDescription;
    }
     ...
     */
    public static VizoGlance fromCursor(Cursor cursor) {
        VizoGlance glance = new VizoGlance();
     ...
     */
    public static ArrayList<VizoGlance> getPendingGlancedItems(Context context) {
        ArrayList<VizoGlance> glances = new ArrayList<>();
        ContentResolver contentResolver = context.getContentResolver();
        ...
        if (cursor != null && cursor.getCount() > 0) {
            while (cursor.moveToNext()) {
                glances.add(VizoGlance.fromCursor(cursor));
            }
        }
     ...
     */
    public static ArrayList<VizoGlance> getStateOfDayGlances(Context context) {
        ArrayList<VizoGlance> glances = new ArrayList<>();
        ContentResolver contentResolver = context.getContentResolver();
        ...
        Cursor cursor = contentResolver.query(DatabaseProvider.GLANCES_URI,
                null, selection, null, DatabaseConstants.MODIFIED_DATE + " DESC");
        if (cursor != null && cursor.getCount() > 0) {
            while (cursor.moveToNext()) {
                glances.add(VizoGlance.fromCursor(cursor));
            }
